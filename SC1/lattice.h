...
public:
    ...
    void returnAg(position& p, double RemoveAgAmount = 1); //For Ag return
    void AddTotalAmountAginLattice(double Agamount); //
public:
    vector < vector < vector < pair < FDC* , pair < vector < int > , double > > > > > FDC_Ag_grid; //   later, replace vector<int> by a type or an ID for an antigen
    double TotalAmountAginLattice; //For observer analyzes
