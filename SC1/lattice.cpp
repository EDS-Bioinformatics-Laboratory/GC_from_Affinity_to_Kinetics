...
lattice::lattice(parameters& p, string CXCL12fname, string CXCL13fname)
  ...
  cz = int((Z - 1) / 2);        // z coordinate of Center
        GCborder = double(p.par[radius] * p.par[radius]);  // radius^2
...
void lattice::putcellat(cell* c) {
  ...
  if (celltypeat(c->position) != empty)
  {
...
vector3D lattice::getFreePosition(double minZperc, double maxZperc) {
  ...
  while ((!foundPlace) && (ocupiedspaces < 10000000)) {
    ...
    if (celltypeat(freePosition) == empty)
    {
      foundPlace = true;
      ocupiedspaces = 0;
      return freePosition;
...
vector3D lattice::get_fdc_position(int j)
{
    ...
    {
        tmp= true;
        cout<<"Using Hyphasma FDC position."<<endl;
    }
    return fdcposes[j];
}
void  lattice::set_initial_fdc_position()
{
    ...
    int b=0;
    int c=0;
    infile.open("fdc_pos_hyphasma.txt");// file containing numbers in 3 columns
       if(infile.fail()) // checks to see if file opended
      {
        cout << "error fdc_position" << endl;
          exit(1);
      }
         while(!infile.eof()) // reads file to end of *file*, not line
        {
           infile >> a; // read first column number
           infile >> b; // read second column number
           infile >> c; // read third column number
            fdcposes.push_back(vector3D(a,b,c));
...
double lattice::getAgat(int x, int y, int z) {
  ...
  if (not(insideBorders(vector3D(x, y, z)))) {
      if (FDC_Ag_grid.at(x).at(y).at(z).second.second>0){
          
    cerr << "Error, get Ag out of borders." << x << " , " << y << " , " << z << endl;
...
void lattice::removeAgAt(position& p, double RemoveAgAmount) {
  ...
  FDC_Ag_grid.at(p.X).at(p.Y).at(p.Z).second.second -=
    ...
    TotalAmountAginLattice -= RemoveAgAmount;
}

void lattice::returnAg(position& p, double RemoveAgAmount) {
  if (not(insideBorders(p))) {
    cerr << "Error, remove Ag out of borders" << p.print() << endl;
    exit(1);
  }
