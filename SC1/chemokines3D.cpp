...
{
    X = int(2 * p.par[radius] / p.par[chemo_dx]) +1;
    Y = int(2 * p.par[radius] / p.par[chemo_dx]) +1;
    Z = int(2 * p.par[radius] / p.par[chemo_dx]) +1;
...
void  chemokines3D::putchemokines3D(molecules whichCXCL,vector3D& pos,double concentration_ )
{
    ...
    if (std::isnan(concentration_)) cerr<<"Cant put chemokine concentration = NAN"<<endl;
...
double chemokines3D::concentrationat(molecules whichCXCL, int x, int y, int z)
{
    ...
    return chemo_grid[x][y][z][whichCXCL]; 
}
