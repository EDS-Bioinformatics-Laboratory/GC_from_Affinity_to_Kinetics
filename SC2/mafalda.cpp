...
//    newFDC->position = l.getFreePosition(0, p.par[zoneRatioGC]);
  ...
  for (unsigned int j = 0; j < p.par[InitialNumberCB]; j = j + 1) {
    ...
    newB_cell->speed = p.par[Bcell_speed];//random::randomNormaldouble(0.18, 0.08);//
    ...
    if (Bcell->cell_type == Centrocyte) {
      switch (Bcell->cell_state) {
        case unselected: {
          ...
          if (Bcell->BC_FDC_interaction_clock > p.par[collectionFDCperiod]) {
            ...
            if (fdc_position.X != -1) {
              ...
              Bcell->setMyAffinity(p);
             //SC-2: Without Interruptions, Binding based on P_on (association probability of CC in shape-space)
        ...
        case contact_FDC: {
         ...
         ///Antigen collection SC-2: Without interuptions, only check P_off (=1-Pd) for dissociation before initiation of Ag extraction
        ...
        case TC_selected: {
	     ...
	     //Bcell->myBCR.pMut = Bcell->myBCR.pMut + (double)(-1.0*0.5*Bcell->myBCR.getMyAffinity4Ag(p));
              Bcell->cyclestate = cycle_G1;
