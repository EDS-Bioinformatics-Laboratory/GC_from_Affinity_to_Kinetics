...
parameters::parameters() {
  ...
  names[kinetics_scenario] = "Declears the scenario that we use to change kon and koff";
  par[kinetics_scenario]= 1.0; //1 both kon koff change with shm, 2 only koff changes, 3 use motility
  ...
  names[pSel] = "     Rate of positive selection at FDCs.    ";
  par[pSel] =
      20;
  ...
  names[Tcell_speed] = "    T-Cell Speed (um / min)    ";
  par[Tcell_speed] = 10;
  names[Tcell_tp] = "    T-Cell Persistent Time average (hr.)    ";
  par[Tcell_tp] = 1.7;  // hour
  names[Tcell_stddev] = "    deviation of T-cell speed (um/min)    ";
  par[Tcell_stddev] = -1;
...
void parameters::writeparameters(string folder,short simulation_id_tmp) {
    
  ofstream myfile;
  string tmp_address = folder + "Parameters_" + to_string(simulation_id_tmp)+".txt" ;
  myfile.open(tmp_address);
...
void parameters::Set_additional_parameters(string additional_parameters_path)
{
    
    // Resizes the vector for paramteres values
//    additional_parameters.first.resize(1000); //names
    additional_parameters.resize(1000); //values
    // Resizes the vector for paramteres names
    int i=0;
    char tmp_name [500];
    double tmp_value;
    if (FILE *add_params_file = fopen(additional_parameters_path.c_str(), "r")) {
//        cout<<"VAl="<<&tmp_name[0]<<endl;
        while (fscanf(add_params_file, "%s %lf", &tmp_name, &tmp_value)==2 ) {
            additional_parameters[i]=(tmp_value);
            i++;
//            additional_parameters.second.push_back(tmp_value);
        }
        fclose(add_params_file);
        cout<<"Reading additional parameters ... is done."<<endl;
    }
    else{   cout<<"Error in reading additional parameters, the file is not open."<<endl;
        exit(1);
    }
//    
