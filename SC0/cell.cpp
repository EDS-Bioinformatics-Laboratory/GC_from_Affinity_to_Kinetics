...
cell::cell(cell* copied_cell) : position(0, 0, 0), polarity(0., 0., 0.) {
  ...
  Born_time=copied_cell->Born_time;
}
...
void cell::getRandomPolarity(parameters& p, lattice& l) {
  ...
  polarity.Y = n[1];
  polarity.Z = n[2];
  if (std::isnan(polarity.X) || std::isnan(polarity.Y) || std::isnan(polarity.Z)) {
    ...
    if (random::randomDouble(1) < speed) {
      ...
      for (unsigned int i = 0; i < neighbours.size(); i++) {
        if ((l.insideBorders(neighbours[i])) &&
            (l.celltypeat(neighbours[i]) == empty))
        {
...
: cell() /*cell with new ID MID = -1 */, myBCR(p) {
   //ID, MID, can_move, speed, persistence time, cell_type and cell_state are set in the cell() constructor
    clonal_id = get_new_clonal_id();
    m_clonal_id = clonal_id;
...
B_cell::B_cell(parameters& p, B_cell* Mom):cell(), myBCR(p) {
    MID = Mom->ID;
    Mom->ID = getNewId(); //This is changed becaue every division creates two new cells, the other cell gets new ID by cell() constructor
    clonal_id = Mom->clonal_id;
    m_clonal_id =Mom->m_clonal_id;
    myBCR = Mom->myBCR;
    myBCR.P_on=Mom->myBCR.P_on;
    myBCR.P_off=Mom->myBCR.P_off;
    myBCR.k_on=Mom->myBCR.k_on;
    myBCR.k_off=Mom->myBCR.k_off;
    myBCR.distance=Mom->myBCR.distance;
    cell_state = Mom->cell_state;
    cell_type = Mom->cell_type;
    persistence_time = Mom->persistence_time;
    speed = Mom->speed;
    can_move = Mom->can_move;
    nFDCcontacts = 0.;
    setMyAffinity(p);
    pMHC_dependent_number_of_divisions = Mom->pMHC_dependent_number_of_divisions;
    total_number_of_divisions = Mom->total_number_of_divisions;
    cyclestate = Mom->cyclestate;
    interactingTC = NULL;
    cycle_state_time = 0.;
    time_of_cycle_state_switch = 0.;
    Bc_Tc_interaction_clock = 0.;
...
void B_cell::mutate(parameters& p) {
   myBCR.mutateBCR(p);  // Mutates BCR
}
...
Plasma_cell::Plasma_cell(parameters& p)
  ...
  Selected_by_FDC=false; //Indicates if B-cell rescued by FDC
  Selected_by_TC=false;
  fdc_binding_nums=0;
  fdc_binding_nums_no_ag=0;
  //Other factors should be feeded in case of creating (not differentiating) Plasma cell
}
...
Plasma_cell::Plasma_cell(parameters& p, B_cell* Bcell) : cell(Bcell), myBCR(p) {
  MID = Bcell->MID;
  ID = Bcell->ID;
  clonal_id=Bcell->clonal_id;
  m_clonal_id=Bcell->m_clonal_id;
  ...
  myBCR.k_on= Bcell->myBCR.k_on;
  myBCR.P_on=Bcell->myBCR.P_on;
  myBCR.P_off=Bcell->myBCR.P_off;
  myBCR.delta_mtheta=Bcell->myBCR.delta_mtheta;
  myBCR.delta_Poff=Bcell->myBCR.delta_Poff;
  myBCR.delta_Pon=Bcell->myBCR.delta_Pon;
  myBCR.k_off= Bcell->myBCR.k_off;
  myBCR.m_theta= Bcell->myBCR.m_theta;
  myBCR.distance=Bcell->myBCR.distance;
  myBCR.delta_Affinity = Bcell->myBCR.delta_Affinity;
  can_move = true;
  ...
  fdc_binding_nums=Bcell->fdc_binding_nums;
  fdc_binding_nums_no_ag=Bcell->fdc_binding_nums_no_ag;
...
Memory_cell::Memory_cell(parameters& p)
    : cell(),
      myBCR(p),
      //   here, you might want to put initial values to Blimp1 and
      // Bach2
      BLIMP1(0),
      IRF4(0) {
  MyAffinity = myBCR.getMyAffinity4Ag(p);
...
void B_cell::proliferate(parameters& p, lattice& l, double time,
  ...
  if (nDivisions2do > 0) {
    ...
    if (freeSpace2Divide.X != -1 && freeSpace2Divide.Y != -1 &&
      ...
      currentoutput.close_event(this,currentsimulation.sim_output,time,currentsimulation.Simulation_ID,currentsimulation.Concept_ID,currentsimulation.Sim_Seed);
      ...
      event.str(string());
      cycle_state_time = 0.;
        myBCR.calculate_on_off_P(p);
//        myBCR.calculate_koff(p);
//        daughter_Bcell->myBCR.calculate_koff(p);
        daughter_Bcell->myBCR.calculate_on_off_P(p);
        Born_time=time;
        daughter_Bcell->Born_time=time;
      ...
      daughter_Bcell->polarity = polarity;
      ...
      event<<ID<<","<<time<<","<<MID<<","<<m_clonal_id<<","<<clonal_id<<",";
      daughter_Bcell->event<<daughter_Bcell->ID<<","<<time<<","<<daughter_Bcell->MID<<","<<daughter_Bcell->m_clonal_id<<","<<daughter_Bcell->clonal_id<< ",";
