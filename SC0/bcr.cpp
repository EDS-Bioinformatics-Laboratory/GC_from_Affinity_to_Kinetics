...
{   /// Initialize mutation counters
    ...
    if (tmp_rnd<0.33)
    {BCReceptor={4,5,4,2};}//initial_affinity_seeds[random::randomInteger(p.par[BCR_pool])];
    else if (tmp_rnd<0.66)
    {BCReceptor={4,5,2,4};}//initial_affinity_seeds[random::randomInteger(p.par[BCR_pool])];
    ...
    initial_affinity_seeds.resize(p.par[BCR_pool],vector<int>(p.par[BCR_Length]));
       for (int j=0;j<p.par[BCR_pool];j++){
           for (int i =0; i< p.par[BCR_Length];i++){
               initial_affinity_seeds[j][i]=random::randomInteger(p.par[Nmax]+1); // default: Nmax is 9 --> each digit is between 0-9 so we use random(10) to include 9 itself
           }}
