...
void simulation::InitialCells(lattice& l, parameters& p) {
  ...
  /*Initialize FDC network,randomley placed in light zone,immobile, transparent,6 directions (2 in each plan) */
...
//    newFDC->position = l.getFreePosition(0, p.par[zoneRatioGC]);
  ...
  for (unsigned int j = 0; j < p.par[InitialNumberCB]; j = j + 1) {
    ...
    newB_cell->myBCR.calculate_on_off_P(p);
//    newB_cell->myBCR.calculate_koff(p);
...
static bool fdc_cxcl_data = false;
  ...
  // Time loop
  for (int counter = 0; counter <= Total_time_steps;
       counter++)
  {
    // Time:
    ...
    switch (Tcell->cell_state) {
      ...
      case TC_connected: {
        ...
        sort(Tcell->interactingCC.begin(), Tcell->interactingCC.end(),
             ...
             });  // sorts descendingly based on retained Ag
        Tcell->polarity.X =
      ...
      switch (Bcell->cell_state) {
        case unselected: {
          ...
          if (Bcell->BC_FDC_interaction_clock > p.par[collectionFDCperiod]) {
            ...
            //SC-0: Binding by Affinity of CC in the shape-space
              double binding_probability = Bcell->myBCR.getMyAffinity4Ag(p);
...
///SC-0: Antigen is extracted after Ag extraction time is over (P=0.04 for Ag extraction time)
        ...
        case contact_TC: {
          ...
          if (Bcell->TCsignalDuration > p.par[tcRescueTime]) {
            ...
            double ndivtmp =2.0;  // total number of divisions hyphasma 
